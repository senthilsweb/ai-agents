...
- The **complete contents of every phase trace JSON** (orchestrate.json + all
  render-*.json). Inline — not file paths.
...
Read `cost-rates.yaml` from the `cost_rates` skill. Use its `mode`:
...
tokens table, a cost table (or an "n/a" line with the reason), a QC line per
variation, and a linked list of artifacts (relative paths). Do not invent
numbers — if something is unknown, print `n/a` and a one-line reason.
