...
// ── Deterministic resume extraction (Docling, in-sandbox exec, no LLM) ────
//
// See openspec/changes/add-job-matcher/design.md. Adapted from
// privacy-classifier's extract_document_text.ts for the single-resume case
// (PDF, DOCX, TXT, or Markdown — no images/HTML, per spec.md "Input
// sources").

const PASSTHROUGH_EXTENSIONS = new Set([".txt", ".md", ".markdown"]);
const DOCLING_EXTENSIONS = new Set([".pdf", ".docx", ".doc"]);
...
export default defineTool({
  description:
    "Deterministically extract plain text from the resume. TXT/Markdown " +
    "pass through directly. PDF and DOCX go through Docling (Python, run " +
    "in-sandbox via exec) with automatic local OCR fallback for scanned " +
    "PDF pages — no hosted API, no LLM. Writes resume.txt to the run " +
    "folder and returns metadata only.",
  ...
  async execute({ run_dir, sandbox_path, extension }, ctx) {
    ...
    const usedOcr = DOCLING_EXTENSIONS.has(ext);
    ...
    if (PASSTHROUGH_EXTENSIONS.has(ext)) {
      ...
      const command = [
        PYTHON_BIN,
        "/workspace/scripts/extract_resume.py",
        sandbox_path,
        textOutPath,
        metaOutPath,
      ]
