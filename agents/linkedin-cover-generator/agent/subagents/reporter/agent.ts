...
// ── Reporter subagent ─────────────────────────────────────────────────────
// Aggregates a run's phase traces into report.md + summary.json with timing,
// token, and cost metrics. Configured independently via MODEL_REPORTER* env
// vars (fallback to MODEL*). A fast, non-reasoning model is ideal here — the
// reporter does structured data aggregation, not open-ended reasoning.
//
...
//
// NOTE: This reporter is intentionally lightweight — it preserves the
// orchestrator + subagent architecture for reference. It receives phase traces
// and run metadata, computes timing/token/cost metrics, and returns the full
// report.md and summary.json content. The orchestrator writes these to the run
// dir and syncs to the host.
