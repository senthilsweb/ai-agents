...
When exactly one job source is supplied, the orchestrator SHALL analyze it directly with one strongly-typed structured-output call and SHALL NOT spawn a subagent. When more than one job source is supplied, the agent SHALL spawn one `job-analyst` subagent per job source with bounded, configurable concurrency, and **each job source SHALL produce its own separate trace**. Every trace belonging to a run — the orchestrator's and each job's — SHALL carry the run id as an attribute so a run's traces can be correlated in the telemetry backends.
...
A run SHALL produce exactly one self-contained JSON report **per job source**, named `slug(<job title>)_<timestamp>.json`, containing run metadata, the job text and extracted resume text, the typed analysis, the computed score breakdown, the match band, a recommendation, and cover-letter content as text fields. V1 SHALL NOT generate DOCX, PDF, or HTML documents. Multi-job runs SHALL additionally write a lightweight ranked summary ordered by total score.
...
Each run SHALL persist its inputs' extracted text, per-job analyses, and per-job JSON reports under a timestamped `runs/` folder, synced to the host and uploaded via the existing shared tools.

## Requirement: Evals
The agent SHALL ship evals covering: per-job report schema conformance (including the `slug(<job title>)_<timestamp>.json` naming contract), scoring-formula determinism, match-band boundaries, evidence grounding, multi-job fan-out with one-trace-per-job verification (N job sources → N distinct trace ids sharing the run id attribute), single-job direct path (no subagent, one trace), prompt-injection resistance, and graceful link-failure handling (single attempt, logged, per-job stop, no retry, remaining jobs unaffected). Deterministic evals (scoring, banding) SHALL run without a live model.
