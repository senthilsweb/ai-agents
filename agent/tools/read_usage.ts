...
export default defineTool({
  description:
    ...
    "and any child subagent sessions. Call this after each subagent returns and " +
    "again before invoking the reporter, so token data can be written into phase " +
    "traces. Returns a map of sessionId → usage. If session_id is given, returns " +
    "only that session's usage.",
  ...
  async execute({ session_id }) {
    if (!existsSync(USAGE_DIR)) {
      return { sessions: {}, note: "No usage data captured yet." };
    }

    if (session_id) {
      const p = join(USAGE_DIR, `${session_id}.json`);
      if (!existsSync(p)) {
        return { sessions: {}, note: `No usage found for session ${session_id}.` };
      }
